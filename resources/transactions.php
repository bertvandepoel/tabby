...
function get_transactions_per_user($debtormail) {
	...
	while($row = $get_debt->fetch(PDO::FETCH_ASSOC)) {
		if(isset($users[$row['user']])) {
			if(count($users[$row['user']]['data']) == 4) { // we won't display more than 4 records but just give a link for the details
				$users[$row['user']]['more'] = TRUE;
				$users[$row['user']]['total'] += $row['amount'];
				continue;
			}
			$users[$row['user']]['data'][] = array('date' => $row['activity_date'], 'sort' => $row['activity_date'] . $row['activity_id'] . $row['comment'], 'description' => $row['activity_name'] . ' - ' . $row['comment'], 'amount' => $row['amount']);
			$users[$row['user']]['total'] += $row['amount'];
		}
		else {
			$users[$row['user']] = array('user' => $row['user'], 'name' => $row['name'], 'email' => $row['email'], 'iban' => $row['iban'], 'total' => $row['amount'], 'more' => FALSE, 'data' => array());
			$users[$row['user']]['data'][] = array('date' => $row['activity_date'], 'sort' => $row['activity_date'] . $row['activity_id'] . $row['comment'], 'description' => $row['activity_name'] . ' - ' . $row['comment'], 'amount' => $row['amount']);
		}
	}
	while($row = $get_credit->fetch(PDO::FETCH_ASSOC)) {
		...
		$users[$row['user']]['data'][] = array('date' => $row['date'], 'sort' => $row['date'] . $row['creditid'], 'description' => $row['comment'], 'amount' => $row['amount']);
		$users[$row['user']]['total'] += $row['amount'];
	}
...
function get_user_transactions_for_debtor($usermail, $debtormail) {
	...
	while($row = $get_debt->fetch(PDO::FETCH_ASSOC)) {
		if(!empty($user)) {
			...
			$user = array('user' => $row['user'], 'name' => $row['name'], 'email' => $row['email'], 'iban' => $row['iban'], 'total' => $row['amount'], 'data' => array());
			$user['data'][] = array('date' => $row['activity_date'], 'sort' => $row['activity_date'] . $row['activity_id'] . $row['comment'], 'description' => $row['activity_name'] . ' - ' . $row['comment'], 'amount' => $row['amount']);
		}
	}
	while($row = $get_credit->fetch(PDO::FETCH_ASSOC)) {
